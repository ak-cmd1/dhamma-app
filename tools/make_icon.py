...
# ---------- 読むだけのアプリのアイコン ----------
# 円相(輪)と一(線)。並べたときに形がまったく違うので、
# ホーム画面で取り違えない。書としても対になる。
def 一本(cx, cy, 長, 幅基, 面, 角=None, 傾=-0.030, 種=0):
    """筆の横画を一本。垂れを作らず、両端を斜めに切る。
       垂れを付けると、小さくしたとき刃物に見えてしまう。"""
    if 角 is None:
        角 = math.radians(-32)
    x0, x1 = cx - 長 / 2, cx + 長 / 2
    y0, y1 = cy - 傾 * 大 / 2, cy + 傾 * 大 / 2
    for i in range(2200):
        t = i / 2199
        x = x0 + (x1 - x0) * t
        y = y0 + (y1 - y0) * t
        if t < 0.04:
            幅 = 幅基 * (0.66 + 0.34 * (t / 0.04) ** 0.5)
        elif t < 0.90:
            幅 = 幅基 * (1.0 - 0.18 * ((t - 0.04) / 0.86) ** 0.9)
        else:
            幅 = 幅基 * (0.82 * max(0.0, (1.0 - (t - 0.90) / 0.10)) ** 0.45)
        濃 = 1.0
        if 0.55 < t < 0.88:
            濃 = 0.88 + 0.12 * abs(math.sin(t * 63.0 + 種))
        置く(面, x, y, 大 * max(幅, 0.003) * 0.5, 濃, 0.54, 角)

def 一(縮=1.0):
    面 = np.zeros((大, 大), np.float32)
    一本(大 * 0.5, 大 * 0.5, 大 * 0.62 * 縮, 0.135 * 縮, 面, 傾=-0.030 * 縮)
    return 面
